...
const ReferralItem: React.FC<IProps & { data: IReferral; isRedeemed: boolean }> = ({ profileData, data, isRedeemed }) => {
  ...
  const showRefferal = useMemo(() => data.referredByUserId === data.referredToUserId && data.referredToUser === globalStore?.currentUser?.uid && data.referredToUserId === profileData.UserId, [globalStore, data]);
