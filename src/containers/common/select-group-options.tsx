...
import { GetAllGroupTypes, IGroupType } from '@/firebase/group-types';
...
const SelectGroupOptions: React.FC<IProps> = ({ handleSelectOption, selectedOption, handleGoBack, notForAuthPage }) => {
  const [isFetching, setIsFetching] = useState(false);
  const [allOptions, setAllOptions] = useState<IGroupType[]>([]);
  const [filteredOptions, setFilteredOptions] = useState<IGroupType[]>([]);
  const [debouncedSearchTerm, setSearchTerm] = useDebounceValue('', 500);

  const handleFetchData = async () => {
    setIsFetching(true);
    const response = await asyncGuard(() => GetAllGroupTypes());
  ...
  useEffect(() => {
    const filtered = allOptions.filter((item) => item.name.toLowerCase().includes(debouncedSearchTerm.toLowerCase()));
  ...
  return (
    <AuthCardLayout title="Select Group" onBack={handleGoBack} containerClassName="gap-2" coverImageSrc="/images/auth-cover-03.jpg" notForAuthPage={notForAuthPage}>
      {isFetching ? (
